...
async function main() {
  ...
  // Precomputed here (rather than filtered client-side against PRODUCTS)
  // so category nav can hide empty categories on product pages too, which
  // never load products-data.js at all — only this file.
...
const CATEGORIES_WITH_PRODUCTS = ${JSON.stringify(categoriesWithProducts, null, 2)};
`;
